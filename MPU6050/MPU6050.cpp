...
void MPU6050::whoAmI()
{
 //  whoAmI = readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050);
}
...
DataVector MPU6050::readGyroData()
{
    ...
    DataVector ret;
    /* Turn the MSB LSB into signed 16-bit value */
    ret.X = (int16_t)(((int16_t)rawData[0]<<8) | rawData[1]);  // GYRO_XOUT
    ret.Y = (int16_t)(((int16_t)rawData[2]<<8) | rawData[3]);  // GYRO_YOUT
    ret.Z = (int16_t)(((int16_t)rawData[4]<<8) | rawData[5]);  // GYRO_ZOUT  
...
void MPU6050::Calibrate()
{
    ...
    for(int i = 0; i<5; i++)
    {
        DataVector ret = readGyroData();
        gyroOff = gyroOff + ret;
        wait(0.11);
    }
    
    GyroOffset.X = gyroOff.X / 5;
    GyroOffset.Y = gyroOff.Y / 5;
    GyroOffset.Z = gyroOff.Z / 5;
}


DataVector MPU6050::GetGyroData()
{
    return readGyroData() - GyroOffset;
}
